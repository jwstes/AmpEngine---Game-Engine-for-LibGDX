...
public class AIManager extends Entity {
    ...
    public AIManager(float initialPosX, float initialPosY) {
        ...
        // Initialize spikeTextures array with 4 spike images
        spikeTextures = new Texture[4];
        for (int i = 0; i < 4; i++) {
    ...
    public void update() {
        ...
        if (!movementPositions.isEmpty()) {
            ...
            if (TimeUtils.timeSinceMillis(lastMoveTime) > movementWaitInterval) {
                fullyErected = (currentSpikeImageIndex == 3);
                if (fullyErected) {
                ...
                currentSpikeImageIndex = (currentSpikeImageIndex + 1) % 4;
